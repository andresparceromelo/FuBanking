...
/**
 * Caso de Uso: Activar 2FA para el usuario autenticado.
 *
 * Requiere que el usuario ya esté autenticado (req.user del authMiddleware).
 * Actualiza el campo two_factor_enabled a true en la base de datos.
 */
export class EnableTwoFactor {
  ...
  async execute(userId: string): Promise<PublicUser> {
    ...
    const updatedUser = await this.userRepository.updateTwoFactor(userId, true);
