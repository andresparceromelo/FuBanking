  ...
  async sendPasswordResetEmail(email: string, resetLink: string): Promise<void> {
    try {
      await this.transporter.sendMail({
        from: `"Banco Digital" <${env.GMAIL_USSER}>`,
        to: email,
        subject: '🔒 Restablecimiento de contraseña — Banco Digital',
        html: this.buildResetPasswordTemplate(resetLink),
      });
    } catch (error: any) {
      throw new AppError(
...
<body>
  ...
  private buildResetPasswordTemplate(resetLink: string): string {
    return `
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
  <div class="wrapper">
    <div class="card">
      ...
      <p class="title">Restablecimiento de contraseña</p>

      <p style="color: #e5e5e5; font-size: 14px; margin-bottom: 24px; line-height: 1.5;">
        Hemos recibido una solicitud para restablecer la contraseña de tu cuenta. Haz clic en el siguiente botón para continuar:
      </p>

      <a href="${resetLink}" class="action-btn">Restablecer contraseña</a>
      ...
      <p class="warning">
        ...
        Ignora este mensaje. Tu cuenta permanece segura.
        Si crees que alguien intentó acceder, por favor contacta a soporte.
      </p>
    </div>
